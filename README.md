...
`./build-micro-wayfire-image.sh`, `./build-micro-wayfire-system-closure.sh`,
...
Build the micro Wayfire graphical desktop system:

```bash
nix build path:.#nixosConfigurations.myhost-micro-wayfire.config.system.build.toplevel
```
...
- The graphical profiles now split into three styles: `myhost` and `myhost-minimal` use X11 + LightDM + Xfce, `myhost-micro-openbox` uses X11 + `ly` + Openbox with LXPanel, and `myhost-micro-wayfire` uses Wayland + `ly` + Wayfire with `wf-shell`. The full desktop and installer also enable `picom`.
- The flake now exposes five installed-system fragments: `myhost` for the full desktop profile, `myhost-minimal` for a lean Xfce desktop profile, `myhost-micro-openbox` for the smaller Openbox-based profile, `myhost-micro-wayfire` for the smaller Wayland-based profile, and `myhost-server` for the minimal server profile.
...
- `myhost-micro-openbox` keeps `ly`, Xorg, Openbox, LXPanel, `pcmanfm`, `obconf`, `lxappearance`, `lxrandr`, NetworkManager with `nm-applet`, Bluetooth with Blueman, and Polkit. It drops LightDM, audio, GVFS, UDisks2, portals, ModemManager, default speech packages, default font bundles, default firmware bundles, Home Manager, the dev toolchain sets, and the broader desktop app bundle.
- `myhost-micro-wayfire` keeps `ly`, Wayfire with `wf-shell` and `wcm`, `pcmanfm`, `lxappearance`, NetworkManager with `nm-applet`, Bluetooth with Blueman, and Polkit. It avoids Xorg entirely and drops LightDM, audio, GVFS, UDisks2, portals, ModemManager, default speech packages, default font bundles, default firmware bundles, Home Manager, the dev toolchain sets, and the broader desktop app bundle.
...
- The micro desktop Disko image outputs are named `myhost-micro-openbox.raw` and `myhost-micro-wayfire.raw`. Both use a `1313M` raw disk image, which leaves about `800M` for the encrypted root partition after the 1 MiB BIOS partition and 512 MiB EFI partition.
